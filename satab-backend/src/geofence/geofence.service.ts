...
export class GeofenceService {
    ...
    // حافظه برای شمارش خروج‌های پیاپی و کول‌دان
    private outsideCounts = new Map<number, number>();     // vehicleId -> n
    private lastViolationAt = new Map<number, number>();   // vehicleId -> ts(ms)
    ...
    async upsertForVehicle(vehicleId: number, dto: CreateGeofenceDto | UpdateGeofenceDto) {
        if (dto.type === 'polygon') {
            ...
            if (
                dto.centerLat == null || dto.centerLng == null ||
                dto.radiusM == null || dto.radiusM <= 0
            ) {
    ...
    /** فقط چک می‌کند (ثبت تخلف اختیاری زیر) */
    ...
    /**
     * چک + اعمال منطق N بار پیاپی خارج و cooldown
     * اگر تخلف قطعی شد، در جدول violations ثبت می‌کند.
     */
    async checkAndRecord(
        ...
        if (cnt >= (geo.outsideN ?? 3)) {
            ...
            if (now - last >= (geo.cooldownMs ?? 60000)) {
                // ثبت در violations (بدون تعریف انتیتی دوم)
                await this.ds.query(
                    `INSERT INTO violations (driver_user_id, vehicle_id, type, meta)
           VALUES ($1, $2, 'geofence_exit', $3::jsonb)`,
                    [
                        driverUserId ?? null,
                        vehicleId,
                        JSON.stringify({
                ...
                // ریست شمارنده تا از اسپم جلوگیری شود
